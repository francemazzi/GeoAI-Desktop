...
void TestQgsAiTreesDetectTool::rejectsMalformedArtifact()
{
  ...
  server.responses
    ...
    << QgsAiTestLoopbackServer::jsonResponse( 200, "OK", QByteArrayLiteral( R"({"status":"completed","artifact":{"downloadUrl":"https://example.test/trees.geojson","sha256":"not-a-hash","sizeBytes":12,"format":"geojson","expiresAt":"2030-01-02T03:04:05Z"}})" ) );
  QVERIFY( server.listen( QHostAddress::LocalHost, 0 ) );
